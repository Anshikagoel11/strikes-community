...
const LeaveServerModal = () => {
    ...
    async function onConfirm() {
        try {
            ...
            console.log("unable to leave the server, try again after some time", error)
        } finally {
            setIsLoading(false)
        }
    }
        ...
        <Dialog open={isModalOpen} onOpenChange={onClose}>
            <DialogContent className="bg-background text-foreground p-0 overflow-hidden max-w-md sm:max-w-md">
                <DialogHeader className="pt-8 px-6">
                    <DialogTitle className="text-2xl text-center font-bold">Leave this server</DialogTitle>
                    <DialogDescription className="text-center text-muted-foreground">
                        Are you sure you want to leave the server <span className="font-semibold">{server?.name}</span>?
                    </DialogDescription>
                    <p className="mt-2 text-center text-sm text-muted-foreground">You will lose access to this server and its channels.</p>
                </DialogHeader>
